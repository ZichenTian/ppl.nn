    ...
    const bool diff = true)
{
    ...
    if (diff) {
        hgemm_ref(A, B, M, N, K, M, N, N, C_ref);
    }

    auto gemm_kernel_func = ppl::kernel::arm_server::neon::hgemm_ndarray_kernel_tn_max8x24_func_table[0][0][1][7][2];
    gemm_kernel_func(A, B, K, M, N, N, C);
...
int main(int argc, char* argv[])
{
    ...
    test_hgemm_kernel(8, 24, 256, true);
    return 0;
}
